	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(cmd.equals("add")) {
			...
			try {
				...
				//request.setAttribute("center", "에러페이지");  //에러페이지를 추가할 경우 Navi에 추가해주어야 함
				e.printStackTrace();
			}
			...
			try {
				...
				request.setAttribute("nav", Navi.userupdate);
			} catch (Exception e) {
				e.printStackTrace();
			}
			...
			try {
				...
				//request.setAttribute("nav", Navi.userdetail);
				response.sendRedirect("user.cacao?cmd=detail&id="+id);
				return;
			} catch (Exception e) {
				e.printStackTrace();
			}
